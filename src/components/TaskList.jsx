...
export default function TaskList({ tasks, selectedDate, onUpdate }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (name.trim()) {
      const newTask = createItem(name.trim(), TYPE.TASK, frequency);
      addItem(newTask);
      setName('');
  ...
  const handleToggle = (task) => {
    ...
    toggleCompletion(task, selectedDate);
    onUpdate();
  };
  ...
  if (!selectedDate) {
    return (
      <div className="bg-white rounded-lg shadow-md p-4">
        <h3 className="text-lg font-semibold mb-4">Úkoly</h3>
        <p className="text-gray-500 text-sm">Vyberte datum v kalendáři pro správu úkolů</p>
      </div>
    );
  }
  
  const selectedDateString = formatDate(selectedDate);
  const isCompleted = (task) => task.completedDates.includes(selectedDateString);
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="mb-4 pb-4 border-b">
          <input
            ...
            placeholder="Název úkolu (např. Učení)"
              ...
              <div className="flex items-center gap-3 flex-1">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-gray-500">
                    {task.frequency === FREQUENCY.DAILY ? 'Denně' : 'Týdně'}
                  </div>
                </div>
              </div>
              <button
